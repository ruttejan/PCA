...
- Jak můžeme vidět z výpisu, jednotlivá hodnocení studentů máme jako řádkové věktory. Proto musíme nejprve data transponovat, abychom měli matici v požadovaném tvaru, tudíž jako matici s odpovědmi studentů jako sloupcové vektory: 
$$A=[a_1 \dots a_{5820}]$$
...
::: txt2
- Pro vytvoření korelační matice musíme, kromě vycentrování, data ještě standardizovat (vydělit směrodatnou odchylkou)
$$ \sigma_j = \sqrt{\frac{1}{5819} \sum_{i=1}^{5820} (a_{ij} - \bar{a_j})^2} $$

- Směrodatná odchylka je v našem případě vektor o rozměru 33 (počet otázek)
$$ a_{ij} = \frac{a_{ij}}{\sigma_j} $$
...
- To samé, co jsme udělali na předešlých slidech, můžeme provést i s kovarianční maticí.
```{julia}
# calculate eigen values and eigen vectors
...
# Vizualizace dat {style="font-size: 28px;"}
...
- Zde se nám povedlo promítnout body do prvních 2 dimenzí s chybou 25%, a do prvních 3 s chybou 22% (s Korelační maticí)

- Pro klasifikaci bychom mohli použít prvních 8 dimenzí s chybou už pouhých 10% (s Korelační maticí)
